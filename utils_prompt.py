...
def build_train_pair_aqua(problems, args, curr_le_data=None):

    examples = []
    #text_example = problems
    if args.prompt_format == "QC-E":
        text_example = problems['question'] + " " + " ".join(problems['options']) + " Solution:"#+ " " + problems['rationale']
        target = problems['rationale']
        last_newline_pos = target.rfind('\n')
        if last_newline_pos != -1:
            target = target[:last_newline_pos]

    elif args.prompt_format == "QCG-A":
        if curr_le_data is not None:
            text_example = problems['question'] + " " +  " ".join(problems['options']) + " Solution:" + curr_le_data + " Answer:"
            target = problems['correct']
        else:
            rationale=problems['rationale'] 
            last_newline_pos = rationale.rfind('\n')
            if last_newline_pos != -1:
                rationale = rationale[:last_newline_pos]
            text_example = problems['question'] + " " +  " ".join(problems['options']) + " Solution:" + rationale+ " Answer:"
            target = problems['correct']
    else:
